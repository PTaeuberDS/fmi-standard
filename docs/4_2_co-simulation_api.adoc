...
<<fmi3CallbackIntermediateUpdate>> can be called by the FMU itself and switches it into <<IntermediateUpdateMode>>.
...
#TODO: Paragraphs use different formats (using the header as the beginning of the sentence (capitalized and non-capitalized) vs. repeating the function name at the beginning of the sentence vs. starting a new sentence without function name at the beginning)