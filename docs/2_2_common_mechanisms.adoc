...
* FMI callback functions are not allowed to call back into the FMU (<<logMessage,fmi3CallbackLogMessage>>, <<fmi3CallbackLockPreemption,fmi3CallbackLockPreemption>>, <<fmi3CallbackLockPreemption,fmi3CallbackUnlockPreemption>>), except <<fmi3CallbackIntermediateUpdate,fmi3CallbackIntermediateUpdate>> which leads to the well-defined state <<IntermediateUpdateMode>> defining which function calls back into the FMU are allowed.
...
The FMU may use the <<earlyReturn>> argument of the <<fmi3DoStep>> function to tell the importer that the FMU needs to return earlier, and the importer may use the callback <<fmi3CallbackIntermediateUpdate>> to signal the FMU that the latter should return earlier.
...
_For example, a call to_ <<fmi3SetContinuousStates>> _will provide the actual states in a vector, and this function has to copy the values into the internal model data structure so that subsequent evaluation calls can utilize these values._
...
_In the right diagram, FMU M3 and FMU M4 are connected in such a way that a real algebraic loop is formed._
_This loop might be solved iteratively, for example with a Newton method._
_In every iteration the iteration variable M4_u is provided by the solver, and via the shown sequence of <<get-and-set-variable-values,`fmi3Set{VariableType}`>> and <<get-and-set-variable-values,`fmi3Get{VariableType}`>> calls, the residual is computed and used by the solver to determine a new value of M4_u._
...
is a time-based periodic Clock which ticks with an arbitrary, but constant <<interval>> starting after an arbitrary <<shift>> both defined by the importer.
The importer informs the FMU about the interval using <<fmi3SetInterval>> in <<InitializationMode>>.
...
is a time-based Clock whose next interval is not yet known right after the Clock just ticked, forcing the importer to call <<fmi3GetInterval>> in every <<EventMode>> or in <<IntermediateUpdateMode>> if <<clocksTicked>> == `fmi3True` in <<fmi3CallbackIntermediateUpdate>>, respectively. This means, a tick of a countdown Clock associated with one model partition can also be requested by another model partition of the same FMU.
...
is activated within the FMU and the importer must call <<fmi3GetClock>> in <<EventMode>> or in <<IntermediateUpdateMode>>, respectively. Output Clocks are intended to enable an FMU to provide a trigger to the outside world, e.g. to a <<triggered, `triggered input Clock`>> of another FMU.
...
<<fmi3GetClock>> is only called for (triggered) output clocks. <<fmi3SetClock>> is never called in Scheduled Execution.
 ...
 * `fmi3IntervalNotYetKnown` is returned for a <<countdown-aperiodic-clock>> for which the next interval is not yet known. The importer must query the next interval at each <<EventMode>> or <<IntermediateUpdateMode>> if <<clocksTicked>> == `fmi3True` in <<fmi3CallbackIntermediateUpdate>>, respectively, for all <<countdown-aperiodic-clock>>s because the next interval may change at any <<EventMode>> or <<IntermediateUpdateMode>>.
