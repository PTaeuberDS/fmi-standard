...
In general multiple rates i.e. multiple model partitions can be defined for a Co-Simulation FMU.
...
In Scheduled Execution mode model partitions typically represent tasks that are scheduled by the environment, e.g. via input clocks.
