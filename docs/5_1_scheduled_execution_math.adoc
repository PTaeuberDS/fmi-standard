...
To guard certain code parts against preemption the FMU may use the callback functions <<fmi3CallbackLockPreemption,fmi3CallbackLockPreemption>> and <<fmi3CallbackLockPreemption,fmi3CallbackUnlockPreemption>> provided by the simulation algorithm when instantiating the FMU via <<fmi3InstantiateScheduledExecution>>.
...
[[fmi3CallbackLockPreemption,`fmi3CallbackLockPreemption`]]
[source, C]
----
