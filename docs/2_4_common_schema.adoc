...
#TODO: Why is the default causality for clocks local, if they can only be inputs and outputs? Should the default value for causality for an fmi3AbstractVariable be removed?
...
#TODO: "except for variables of type Clock" should be removed here and a general remark should be added at the beginning of the description of "start" that the following is true for for all variables but clocks, because clocks do not have a start value.
